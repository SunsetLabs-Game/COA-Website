...
const specializations: Specialization[] = [
  {
    icon: (
      <svg
        ...
        aria-label="Digital Warfare Icon"
      >
        <path
          fill="currentColor"
...
export function SyndicateSpecializations() {
  const cardStyles =
    "p-6 rounded-lg border border-[rgba(0,255,255,0.2)] bg-[#0B1B32]";
  const containerStyles =
    "border border-[#0a3653] rounded-xl p-8 bg-[rgb(var(--background-start-rgb))]";
            ...
            {specializations.map((spec, index) => (
              <div key={index} className={cardStyles}>
                <div className="flex flex-col space-y-4">
                  <div className="w-12 h-12 rounded-full bg-[rgba(0,255,255,0.1)] flex items-center justify-center">
                    {spec.icon}
                  </div>
